...
socket3.on('ultima-temperatura', function (data) {
	if (data.estado >20) socket3.emit('alerta-temperatura', {
		estado: "temperatura por encima del limite",
		fecha: new Date()
	});
	else if (data.estado < 10)socket3.emit('alerta-temperatura', {
		estado: "temperatura por debajo del limite, sube la persiana",
		fecha: new Date()
	});
	else socket3.emit('alerta-temperatura', {
...
socket3.on('ultima-luminosidad', function (data) {
	if (data.estado > 20) socket3.emit('alerta-luminosidad', {
